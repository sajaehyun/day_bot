...
def analyze_next_day(ticker):
    ...
    try:
        ...
        if 25 <= rsi < 30: raw += 20; signals.append(f"RSI 과매도반등({rsi})")
        elif 30 <= rsi <= 40: raw += 15; signals.append(f"RSI 반등구간({rsi})")
        elif rsi > 75: raw -= 15; signals.append(f"RSI 과열({rsi})")
        if len(c) >= 35:
            ...
            if len(hist) >= 3:
                ...
                if h2 <= 0 and h3 > 0: raw += 20; signals.append("MACD 골든크로스")
                elif h1 < h2 < 0 and h3 > h2: raw += 12; signals.append("MACD 반전중")
                elif h2 >= 0 and h3 < 0: raw -= 15; signals.append("MACD 데드크로스")
        vol_ratio = 1.0
        if len(v) >= 21:
            avg_v = float(v.rolling(20).mean().iloc[-1])
            vol_ratio = round(float(v.iloc[-1])/avg_v, 2) if avg_v > 0 else 1.0
            if vol_ratio >= 2.0 and abs(chg) < 2: raw += 18; signals.append(f"거래량급증({vol_ratio}x)")
            elif vol_ratio >= 1.5: raw += 8; signals.append(f"거래량증가({vol_ratio}x)")
        ...
        if price > ma5 > ma20: raw += 12; signals.append("정배열")
        elif price < ma5 < ma20: raw -= 10; signals.append("역배열")
        if len(c) >= 20:
            bm = float(c.rolling(20).mean().iloc[-1])
            bs = float(c.rolling(20).std().iloc[-1])
            if price <= bm - 2*bs: raw += 12; signals.append("볼린저하단 반등기대")
        if len(c) >= 5:
            recent = c.tail(5).pct_change().dropna()
            dd = sum(1 for x in recent if x < 0)
            if dd >= 3 and chg > 0: raw += 15; signals.append(f"{dd}일하락후반등")
            elif dd >= 4: raw += 8; signals.append(f"{dd}일연속하락")
        score = max(0, min(100, raw))
        if score >= 70: grade, gk = "강력매수", "strong_buy"
        elif score >= 55: grade, gk = "매수", "buy"
        elif score >= 40: grade, gk = "관심", "watch"
        elif score >= 25: grade, gk = "중립", "neutral"
        else: grade, gk = "회피", "avoid"
...
def scan_all():
    ...
    lines = [f"<b>내일상승예측</b> {at}",""]
    for i,r in enumerate(top[:10],1):
        sig = " | ".join(r["signals"][:3])
        lines.append(f'{i}. <b>{r["ticker"]}</b> {r["grade"]} {r["score"]}점 | \ ({r["change_1d"]:+.1f}%)\n  {sig}')
    send_telegram("\n".join(lines))
